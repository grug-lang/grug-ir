...
* **Human Readability Over Strict TAC:** The `.grir` (grug IR) representation moves away from strict [Three-Address Code](https://en.wikipedia.org/wiki/Three-address_code) (TAC) to prioritize human readability. By allowing variables and constant arguments to be passed directly within function calls (e.g., `t1: number = min(10, 5)`, known as [ANF](https://en.wikipedia.org/wiki/A-normal_form)) rather than utilizing stack pushes, the format remains linear and intuitive. This atomic argument structure eliminates the need for recursive descent parsing in the compiler backend. The `.grbc` (grug bitcode) representation that `.grir` can losslessly translate to and from might still use TAC for performance.
* **Generic Storage:** Generics exist solely for the frontend to perform type-checking. During compilation to `.grir` and `.grbc`, generic types such as `List[number]` are simplified and stored explicitly as `u64` IDs rather than as complex structures.
...
The `program.c` file at the root of the repository merges `creeper-Entity.ll` with `host_fns.ll` at runtime to `tests/minmax/.output/mods.ll` using LLVM's C API. The file `tests/minmax/expected/mods.ll` uses [FileCheck](https://llvm.org/docs/CommandGuide/FileCheck.html) to verify that LLVM successfully optimized the asserts in `creeper-Entity.grug` away, confirming they always hold true:
```ll
